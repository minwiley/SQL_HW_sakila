...
ALTER TABLE actor
    MODIFY description TINYBLOB;
SELECT * FROM actor;

ALTER TABLE actor
   MODIFY description LONGBLOB;
...
SELECT COUNT(last_name), last_name AS "Surnames" FROM actor
    GROUP BY last_name
    ORDER BY last_name;
...
SELECT COUNT(actor_id), title FROM film f
	INNER JOIN film_actor fia
    ON f.film_id = fia.film_id
    GROUP BY title
    ORDER BY COUNT(actor_id) DESC;

-- 6d. How many copies of the film Hunchback Impossible exist in the inventory system?
SELECT COUNT(inventory_id), title FROM film f
